...
from mtg_collection_tools.commands.agent import app as agent_app
...
def load_config(ctx: typer.Context):
    # make an exception if the init command is called
    if ctx.invoked_subcommand == "init":
        return

    if not get_config_path().exists():
        logging.error(
            message=f"No config file found at '{get_config_path()}'. Try running 'mtg init' for initial setup",
            fail=True,
        )
    ...
    try:
        ...
        logging.error(
            message=f"Config file '{get_config_path()}' has validation errors. Please try manually correcting these errors, or run 'mtg init -f' to reinitialize. Errors: \n{e}",
            fail=True,
        )
